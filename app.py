...
from src.api.models.preset import ImageSize, PresetSettings, Preset
from src.utils import get_filename_without_extension, read_settings_file, is_supported_format_file
...
def print_presets() -> None:
    """
    Fetches and prints the available presets from the Dehancer API client.
    ...
    for idx, preset in enumerate(available_presets, 1):
        logger.info(f"[{idx}]\t{preset.caption}")


def print_contacts(file_path: str) -> None:
    """
    Creates contacts for an image.
    ...
    Example:
        ...
        ...
    """
    logger.info(f"Create contacts for the image '{file_path}':")
    available_presets = dehancer_api_client.get_available_presets()
    ...
    for idx, preset in enumerate(requested_presets, 1):
        ...
        logger.info(f"{idx}. '{preset}' : {image_url}")
...
def __process_image(file_path: str, preset: Preset, preset_settings: PresetSettings, preset_number: int) -> None:
    """
    Uploads an image, applies the given preset with specified settings and downloads the processed image.
    ...
    logger.info(f"{preset_number}. '{preset.caption}' : {image_url}")
...
def develop_images(path: str, preset_number: int, custom_preset_settings: Dict[str, float]) -> None:
    ...
    if isfile(path):
        ...
        for filename in os.listdir(path):
            ...
            if isfile(file_path) and is_supported_format_file(file_path, IMAGE_VALID_TYPES):
...
@cli.command()
@click.argument("input")
@click.option("-p", "--preset", "preset", type=int, required=True, help="Preset number.")
@click.option("-s", "--set_contrast", "contrast", type=int, help="Contrast setting.")
@click.option("-e", "--set_exposure", "exposure", type=int, help="Exposure setting.")
@click.option("-t", "--set_temperature", "temperature", type=int, help="Temperature setting.")
@click.option("-i", "--set_tint", "tint", type=int, help="Tint setting.")
@click.option("-b", "--set_color_boost", "color_boost", type=int, help="Color boost setting.")
@click.option("-settings", "--settings_file", type=click.Path(exists=True), help="Settings file.")
@click.option("-l", "--logs", type=int, default=0, help="Enable debug logs (1 for enabled, 0 for disabled).")
def develop(input, preset, contrast, exposure, temperature, tint, color_boost, settings_file, logs):
    if logs == 1:
        enable_debug_logs()
    settings = {}
    if settings_file:
